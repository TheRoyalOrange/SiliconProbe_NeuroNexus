% OpenEphys_BaseAnalysis_Bundled.m
%
% Description: Bundled/batch version of OpenEphys_BaseAnalysis.m. Loops
%   over some or all of one animal's OpenEphys recordings (one per
%   stimulus condition in stim_all) and, per recording, loads raw data,
...
%   spike-detection and TF settings, once per recording in the loop.
%   ProbeInfo is created/loaded once and reused across all recordings in
%   the batch. This is the batch entry point for the first stage of the
%   PreProcessing_StimData pipeline (use OpenEphys_BaseAnalysis.m for a
%   single recording).
%
% Inputs:
%   data (OpenEphys recordings, .Rhythm Data format, loaded via
%     uipickfiles, one per entry in stim_all) - some or all of one
%     animal's recordings. A recording is a consecutive series of trials
%     for a given condition. Starting/stopping a recording without
%     changing the file name or condition creates subfolders within it.
...
%   stim_all (cell array of strings) - PROVIDED BY USER. Name of the
%     stimulus for each recording in the batch, same order as the
%     directories chosen via uipickfiles. Used as part of all output
%     file names.
...
%     number of probes, brain areas per probe, etc.) — applied to every
%     recording in the batch.
%
% Outputs (one set per recording, ProbeInfo shared across the batch):
...
%                                 subplots). There is one per probe
%
% Dependencies: None from this pipeline — this is the batch entry point
%   for the first script run per animal (PreProcessing_StimData stage);
%   it is the looped/batch counterpart of OpenEphys_BaseAnalysis.m (use
%   that one for a single recording). Requires OpenEphys MATLAB tools on
%   the path (uipickfiles, Session/continuous data readers) plus the
%   CSDallshank and slanCM helper functions.
%
%
%
%
...
stim_all = {
            ...
            % 'whisker_TTX_4'
            % 'whisker_TTX_5'
            % 'light22_40-n-whisker_TTX'
            % 'light22_40-n-whisker_TTX_2'
            % 'light22_40-n-whisker_TTX_3'
            % 'light22_40-n-whisker_TTX_4'
            % 'light22_40-n-whisker_TTX_5'
            % 'light22_40_TTX'
            % 'light22_40_TTX_2'
            % 'light22_40_TTX_3'
            % 'light22_40_TTX_4'
...
directory_all = uipickfiles('FilterSpec',load_directory,'Prompt', 'Choose Folders Containing Recordings you listed (in order) [IMMEDIATE SUBFOLDERS SHOULD BE -Record Node ###-]'); %loads file names'E:\Roy\Ungrouped Whiskers\20251011-p11\20251011-p11-light4l22_40'; %for example

%% check directory and stim have same dims
length(stim_all) == length(directory_all)
...
%thresh = 100; %name a threshold value that indicates a stim trigger
...
tr_keep = 1:length(stim_times); %full list of trial indices extracted from this recording. Never edited after this point
...
clear data data_cat data_prefAngle data_tf data_tfphase dataX chan_cat chan_dat itpc phase prefAngle tf wavelet waveletX wavtime wittled_braindat as power
