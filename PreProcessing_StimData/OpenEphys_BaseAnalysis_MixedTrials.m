...
%%%Experiment details%%%
animal = '20260803-p12'; %animal id (suggested format YYYYMMDD-p#, where p# is age). This will be the beginning of all file names
stim = 'light22_56'; %whatever you call this stim. It will be part of all file names so choose wisely
stim_num = 8;

%What are the condition labels for each trial in the recording(s)?
stim_Index = [2, 1, 8, 3, 5, 4, 7, 6, 9, 1, 4, 9, 3, 7, 8, 5, 2, 6, 6, 2, 5, 8, 7, 3, 9, 4, 1, 9, 6, 7, 4, 5, 3, 8, 1, 2, 5, 1, 7, 3, 6, 9, 8, 2, 4];
...
%  numstims1(group) = length(stim_times);
% % 
%  normbineTTL = data(ProbeInfo.TTLch,:)'./max(data(ProbeInfo.TTLch,:)');
%  stim_times = find(normbineTTL >= .4);
%  numstims2(group) = length(stim_times);
% % 
%  stim_times2 = stim_times(stim_Index == stim_num);
...
for i = 1:ProbeInfo.Chans
    ...
    geoenginned_ch = reshape(channel_data, length(stim_times),dur);
    stim_lfp_stimchunks_groups(group,:,:,i) = geoenginned_ch; %with dimensions [trials x epoch length x channels]
end
...
for i = 1:size(wittled_braindat,1)
    ...
    stim_spike_stimchunks_group(group,:,:,i) = geoenginned_ch; %with dimensions [trials x epoch length x channels]
    %bandpassed_stimchunks_group(group,:,:,i) = geoband_ch;
end


end
stim_spike_stimchunks = [];
%bandpassed_stimchunks = [];
